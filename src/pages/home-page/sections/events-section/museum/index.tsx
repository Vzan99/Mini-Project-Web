...
export default function MuseumSection() {
  ...
  const sectionTitle = "Museums";

  const fetchEvents = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/admin/sections?category=Museum`
      );
      ...
      if (eventDetails.length >= 0) {
        ...
        console.error("Museum events is not an array:", eventDetails);
        setEvents([]);
      }
    } catch (err) {
      console.error("Error fetching museum events:", err);
      setEvents([]);
    } finally {
    ...
    >
      <div className="mb-8">
        ...
        <div className="hidden md:flex justify-between items-center">
          ...
          <Link href="/discover?category=Museum">
